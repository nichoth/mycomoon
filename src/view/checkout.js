...
class Checkout extends Component {
    ...
    componentDidUpdate () {
        if (!this.state.order && !this.state.error) {
            paymentForm.build();
        } 
    }
    ...
    render () {
        ...
        if (self.state.order) {
            ...
            return html`<div class="success">
                ...
                <${OrderSummary} order=${self.state.order}
                    lineItems=${self.state.order.lineItems} />
            </div>`
        }
...
function createPaymentForm (self, cart, shipping) {
